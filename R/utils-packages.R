...
get_unloaded_packages <- function(script) {
  packs <- stringr::str_extract_all(string = script, pattern = "(?<=(library|require)\\()[[:alnum:]\\.'\"]+(?=\\))")
  packs <- unlist(packs)
  packs <- stringr::str_replace_all(string = packs, pattern = "\"|'", replacement = "")
  packs <- stringr::str_trim(string = packs)
